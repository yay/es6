...

{
  ...
  const countFolders = (folders) => {
    return folders.reduce((acc, cur) => acc + countChildren(cur), 0);
  };

  const countChildren = (folder) => {
    return folder?.children
      ? folder.children.reduce((acc, cur) => acc + countChildren(cur), 1)
      : 0;
  };

  console.log("folder count:", countFolders(folders)); // 4 objects with `children` field
}
